...
# Run clippy on a specific crate (e.g. `just clippy single_player`)
clippy CRATE:
...
# Build a specific crate (e.g. `just build single_player`)
build CRATE:
  cargo build -p {{CRATE}}

# Test a specific crate (e.g. `just test single_player`)
test CRATE:
  cargo test -p {{CRATE}}
...
serve-web:
  ...
  scripts/dev_server.ts ./dist -p 3000
